...
#animate(p1, nframes=nrow(slice_p), renderer = ffmpeg_renderer())
#animate(p1, nframes=nrow(slice_p), renderer = file_renderer('/Users/kkamundson/Desktop/wr674/class/fancy_vis/animation/'))[1:6]
...
## Let's see if we can see weather fronts by downloading all Colorado data and trying to get those to flash
...
#NOTE TO MATT: I could not get any of the animation tools to work on my computer with the R version I have. I tried several different things - many of which we discussed in class. Therefore I am writing the code as best I can - but can not actually run it. 

#Correlation, so going to have air force academy on one axis and fort collins on the other.
#There will be three lines - each colored by min, max, and mean.
...
p <- daily_p %>%
  group_by(station) %>%
  mutate()
  gather(key='station',value='mean_temp') %>%
  filter(!is.na(value))

ggplot(daily_p,aes(x=station,y=af,color=type)) + 
  geom_point() +
  theme_few()

animate(p, nframes(nrows(daily_p)))
...
tm_shape(co) + 
  tm_polygons() + 
  tm_shape(co_elev) +
  tm_raster(style='cont',palette = 'Browns') + 
  tm_shape(sf_p) + 
  tm_dots(col='station',size=3,palette=c('red','white'))
